...
def compile_model(model):
    ...
    """
    opt = keras.optimizers.Adam(learning_rate=3e-6)
...
def train_model(model, train_data, train_targets, epochs):
    ...
    """
    history = model.fit(train_data, train_targets, epochs=epochs, validation_split=0.2, batch_size=10, callbacks=[checkpoint], verbose=1)
...
checkpoint = ModelCheckpoint(filepath=checkpoint_path, save_weights_only=False, save_freq='epoch', monitor='val_loss', save_best_only=True,verbose=1)
...
y_train = np.array(P.y_train[:154])
y_train = np.reshape(y_train, y_train.shape[0])

# creating dictionnary
d = {'y/d': P.X_train['y/d'][:154], 'pred_P': validate_batch, 'y_train': y_train}
...
d = {'y/d': P.X_train['y/d'][:154], 'pred_P': validate_batch, 'y_train': y_train, 'mse': np.square(validate_batch-y_train)}
output_df_validation = pd.DataFrame(data=d).sort_values(by=['y/d'])
